"""Square shape with all sides equal and all angles 90 degrees."""
...
class Square(ShapeBase, ValidationMixin, TornEdgesMixin, NotchesMixin):
  """Square shape with equal sides and 90-degree angles.

  A square is the simplest quadrilateral with all sides equal and all angles at 90 degrees.
  ...
  def build_shape(self) -> Sequence[VectorPort]:
    """Build square shape with optional decorations.

    Returns:
      Sequence of VectorPort objects defining the square boundary
  ...
  def _build_base_shape(self) -> list[VectorPort]:
    """Build basic square vertices without decorations.
    ...
    """
    return self._make_rect_aligned(self.side, self.side)
  ...
  def get_edge_angle(self, edge_index: int) -> float:
    ...
    Returns:
      Angle in radians (all edges are perpendicular for squares)

    For squares, all edges are axis-aligned:
  ...
  def _apply_decorations(self, vertices: list[VectorPort]) -> list[VectorPort]:
    """Apply torn edges and notches to the base square.

    Args:
      vertices: Base square vertices
  ...
  def _validate_dimensions(self) -> None:
    """Validate that the side length is positive and within acceptable range."""
