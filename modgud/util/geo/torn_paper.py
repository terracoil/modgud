...
class TornPaper(ShapePort):
  ...
  def build_shape(self) -> Sequence[VectorPort]:
    ...
    North and South edges use matching tear patterns for fitting.
    East and West edges use matching tear patterns for fitting.
  ...
  def _validate_parameters(self) -> None:
    ...
    # Validate that amplitude doesn't exceed half the smaller dimension
    min_dim = min(self.width, self.height)
    if self.amplitude > min_dim / 2:
      raise ValueError(
        f'amplitude ({self.amplitude}) must not exceed half the smaller dimension ({min_dim / 2})'
      )

  def _create_paper_outline(self, torn_sides: str) -> VectorPath:
    """Create the complete paper outline with torn edges using margin-based positioning.

    For torn edges, the base edge is positioned inward by the amplitude. This ensures
    that noise displacement (which ranges from -amplitude to +amplitude) naturally
    stays within bounds without needing to clamp.

    For example, if North edge is torn with amplitude=5:
    - Base edge is at y=5 instead of y=0
    - Noise can displace from y=0 to y=10, staying within [0, height]
    """
    # Calculate margin offsets for each side based on whether it's torn
    # Margins ensure noise displacement stays within bounds
    north_margin = self.amplitude if 'N' in torn_sides else 0.0
    south_margin = self.amplitude if 'S' in torn_sides else 0.0
    east_margin = self.amplitude if 'E' in torn_sides else 0.0
    west_margin = self.amplitude if 'W' in torn_sides else 0.0

    # Define paper corners with margins applied (clockwise from top-left)
    corners = [
      Vector(west_margin, north_margin, name='topLeft'),  # Top-left
      Vector(self.width - east_margin, north_margin, name='topRight'),  # Top-right
      Vector(
        self.width - east_margin, self.height - south_margin, name='bottomRight'
      ),  # Bottom-right
      Vector(west_margin, self.height - south_margin, name='bottomLeft'),  # Bottom-left
    ]

    # Collect all absolute positions for the path
    all_segments: list[VectorPort] = []
    ...
    for side, start_corner, end_corner, edge_name in edges:
      if side in torn_sides:
        # Create noisy line with adjusted amplitude for fitting
  ...
  def _get_fitting_amplitude(self, base_amplitude: float, side: str) -> float:
    ...
    """
    fitting_signs = {
      'N': 1,  # North tears away from center (positive y displacement)
      'S': -1,  # South tears opposite to North
      'E': 1,  # East tears away from center (positive x displacement)
