...
class Pipeable(PipeableProtocol):
  ...
  def __call__(self, *args: Any, **kwargs: Any) -> Union['Pipeable', Any]:
    ...
    try:
      ...
      if (
        not self._bound_args
        and not self._bound_kwargs
        and args
        and len(param_names) > 1
        and total_args_provided + 1 <= total_params  # +1 for pipeline input
        and not (len(param_names) == 2 and param_names[0] in ('args', 'arguments'))
      ):
    ...
    try:
      ...
      if (
        len(param_names) == 2
        and param_names[0] in ('args', 'arguments')
        and param_names[1] in ('kwargs', 'keywords')
        and (args or kwargs)
      ):
    ...
    try:
      ...
      error_str = str(e)
      if (
        ('missing' in error_str and 'required' in error_str)
        or ('takes' in error_str and 'positional' in error_str)
        or ('multiple values for argument' in error_str)
      ):
      ...
      error_str = str(e)
      if (
        'GuardClauseError' in str(type(e))
        or 'must be positive' in error_str
        or 'cannot be empty' in error_str
        or 'must be of type' in error_str
      ):
        ...
        if not self._bound_args and self._bound_kwargs and len(param_names) > 1:
          ...
          for i in range(1, len(param_names)):  # Start from 1 to skip pipeline parameter
            ...
            for j in range(1, i):  # Check all earlier parameters
              ...
              if (
                earlier_param in kwargs_keys
                and param_name in kwargs_keys
                and kwargs_keys.index(earlier_param) > kwargs_keys.index(param_name)
              ):
