...
@pipeable
@implicit_return
@guarded_expression(positive('x'), not_none('rate'))
def add_tax(x, rate=0.1):
  """Add tax with guards and implicit return."""
  x * (1 + rate)
...
class TestPipeableWithGuardedExpression:
  ...
  def test_guard_validates_all_params(self):
    ...
    # Test that guards are checked even during partial application
    # Trying to create multiply_guarded("not a number") fails immediately
    # because positive('x') tries to check if "not a number" > 0
    with pytest.raises(TypeError, match="'>' not supported"):
      multiply_guarded("not a number")
...
class TestDecoratorOrdering:
  ...
  def test_implicit_return_marker_preserved(self):
    ...
    assert hasattr(add_tax, '__implicit_return__')
    assert hasattr(calculate_discount, '__implicit_return__')
...
class TestRealWorldExamples:
  ...
  def test_financial_calculation_pipeline(self):
    ...
    @pipeable
    @guarded_expression(positive('amount'), positive('fee'))
